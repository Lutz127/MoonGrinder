...
export class CatalogModel {
  ...
  applyFilters(levels, filters = {}, state) {
    ...
    const difficultyMoonGroups = new Set(filters.difficultyMoonGroups || []);
    ...
    return levels.filter((level) => {
      ...
      if (difficulties.size || difficultyMoonGroups.size) {
        const splitKey = `${level.difficulty}|${Number(level.moons) || 0}`;
        if (!difficulties.has(level.difficulty) && !difficultyMoonGroups.has(splitKey)) return false;
      }
